...
USER_NAME="${USER_NAME:-wildon-observer}"
CONTEXT_NAME="${CONTEXT_NAME:-${K3S_OBSERVER_CONTEXT:-wildon-observer}}"
...
usage() {
  cat <<USAGE
Usage:
  scripts/ops/wireguard/bootstrap-observer-kubeconfig.sh [options]

Options:
  --token <token>             Observer service-account token (or set TOKEN env)
  ...
  --user-name <name>          kubeconfig user name (default: wildon-observer)
  --context-name <name>       kubeconfig context name (default: wildon-observer)
...
if kubectl auth can-i get pods -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "observer context configured and access check passed"
else
  echo "observer context configured, but RBAC access check failed"
  exit 1
fi
